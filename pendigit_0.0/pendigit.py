...
data_train = np.loadtxt("pendigits.tra", delimiter=',')
data_test = np.loadtxt("pendigits.tes", delimiter=',')

print data_train.shape
print data_test.shape

data_X = data_train[:,0:16]
data_Y = data_train[:,16:17]

data_X_test_ = data_test[:,0:16]
###data_X_test = np.concatenate([data_X,data_X_test])
data_Y_test_ = data_test[:,16:17]
data_X_test = data_X
data_Y_test = data_Y
####data_Y_test = np.concatenate([data_Y,data_Y_test])
...
for kkk in range(ite_number):
    ...
    """
    def loss(beta):
        score = np.array([np.dot(beta,data_X_test[i:i+1,:][0,]) for i in range(sample_number)])
        score2 = 1.0/(1.0+np.exp(-score))
        score3 = score2+ratio*(reward-score2)
    ...
    def loss(beta):
        score = np.array([np.dot(beta[1:],data_X_test[i:i+1,:][0,]) for i in range(sample_number)])
        score2 = 1.0/(1.0+np.exp(-score))
        score3 = beta[0]+score2+ratio*(reward-beta[0]-score2)
        return np.mean(score3*score3)
    
    minmin = minimize(loss,np.zeros(sample_dim+1),method='BFGS')
    beta = minmin.x
    ...
    mdr2_list.append(np.mean(score3))
